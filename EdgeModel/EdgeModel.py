...
db = Database()
db.open('banco.db', True)
db.create_tables([TipoPessoa, Pessoa])

aluno = TipoPessoa()
aluno.desc.set_value('Aluno')
if aluno.save():
    print 'saved aluno'

print 'new id aluno:', aluno.id_tipo_pessoa.get_value()

aluno.desc.set_value('Student')
if aluno.save():
    print 'update aluno'

inspetor = TipoPessoa()
inspetor.id_tipo_pessoa.set_value(aluno.id_tipo_pessoa.get_value())
if inspetor.load():
    print 'inspetor loaded'

print 'inspertor ', inspetor.desc.get_value()

if aluno.delete():
    print ('deleted aluno')

professor = TipoPessoa()
professor.desc.set_value('Professor')
if professor.save():
    print('saved professor')


joed = Pessoa()
joed.nome.set_value('joed')
joed.id_tipo_pessoa.set_value(professor)
joed.save()

exit(1)

aline = Pessoa()
aline.nome.set_value('aline')
aline.id_tipo_pessoa.set_value(aluno)
aline.save()
...
TODO
FOREIGN CLASS
    -> set classe Model Pai
    -> keys do Pay para o filho
    -> get -> retorna objeto
    -> set -> passa como parametro o objeto, e seta ids
SELECT
    -> get all with params
    -> join com classe estrangeira
'''
